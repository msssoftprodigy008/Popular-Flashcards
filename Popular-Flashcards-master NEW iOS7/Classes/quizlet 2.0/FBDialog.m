...
static NSString* kDefaultTitle = @"Connect to Quizlet";

static CGFloat kFacebookBlue[4] = {0.0, 44.0/255, 122.0/255.0, 1.0};
...
- (void)sizeToFitOrientation:(BOOL)transform {
  ...
  if (FBIsDeviceIPad()) {
    // On the iPad the dialog's dimensions should only be 95% of the screen's
    scale_factor = 0.95f;
  }
  ...
  if (UIInterfaceOrientationIsLandscape(_orientation)) {
      if (FBIsDeviceIPad()){
          self.frame = CGRectMake(kPadding, kPadding, 768, 480);
      }else{
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request
    ...
    NSLog(@"urlStr %@",url);
    NSString *redirectUrl = [_params objectForKey:@"redirect_uri"];
    NSString *host1 = [url host];
    NSString *host2 = [[NSURL URLWithString:redirectUrl] host];
    if ([host2 isEqualToString:host1]) {
        NSString *errorMsg = [self getStringFromUrl:urlStr needle:@"error="];
        if (errorMsg) {
            NSLog(@"%@",errorMsg);
            errorMsg = [self getStringFromUrl:urlStr needle:@"error_description="];
            if (errorMsg) {
                NSDictionary * errorData = [NSDictionary dictionaryWithObject:errorMsg forKey:@"error_msg"];
                NSError * error = [NSError errorWithDomain:@"quizletErrDomain"
                                                       code:401
                                                    userInfo:errorData];
